...
export default function PingToolPage() {
  ...
  async function handleExecute() {
    ...
    try {
      ...
      if (!response.ok || !Array.isArray(data?.results)) {
        throw new Error(data?.error || "Falha ao executar o ping.");
      }
  ...
  return (
    <DashboardShell
      pageTitle="Ping ICMP"
      pageSubtitle="Verifique latência simulada e disponibilidade de hosts."
    >
        ...
        >
          <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-lg">Ping ICMP</CardTitle>
                <p className="text-sm text-zinc-500">
                  Envie lista de IPs para simular latência média.
                </p>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4 text-sm">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {state.isRunning ? "Executando..." : "Executar ping"}
                </Button>
              </div>
              <p className="text-[11px] text-zinc-500">
                Informe uma lista de IPs ou importe um arquivo .txt/.csv.
              </p>
              {state.fileName && (
                <p className="text-[11px] text-zinc-500">
