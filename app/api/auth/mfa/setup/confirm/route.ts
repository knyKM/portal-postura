...
import { enableUserMfa, findUserById, updateUserLastSeen } from "@/lib/auth/user-service";
...
export async function POST(request: Request) {
  const body = (await request.json().catch(() => null)) as ConfirmSetupRequest | null;
  ...
  if (!user.mfa_secret) {
    return NextResponse.json(
      { error: "Segredo MFA não disponível. Reinicie o processo." },
      { status: 400 }
    );
  }
  ...
  if (user.mfa_enabled) {
    return NextResponse.json(
      { error: "Este usuário já possui MFA configurado." },
      { status: 400 }
    );
  }
