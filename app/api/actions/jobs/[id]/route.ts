...
import { updateActionRequestExecutionStatus } from "@/lib/actions/action-request-service";
...
export async function PATCH(
  ...
  const payload = (await request.json().catch(() => null)) as
    | { paused?: boolean; resume?: boolean }
    | null;

  if (!payload || (typeof payload.paused !== "boolean" && !payload.resume)) {
