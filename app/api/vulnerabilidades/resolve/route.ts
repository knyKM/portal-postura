...
export async function POST(request: Request) {
  ...
  const link = resolveVulnerabilityForServer(vulnerabilityId, serverId);
  if (!link) {
    return NextResponse.json({ error: "Vínculo não encontrado." }, { status: 400 });
  }
