  ...
  return db
    .prepare<ContractRecord>(
      `SELECT id, title, vendor, owner, area, lpu, contract_type, segment, sap_contract, contract_year,
              lpu_image, contract_scope, management, payment_type, payment_schedule_json,
  ...
  const record = db
    .prepare<ContractRecord>(
      `INSERT INTO contracts
        (title, vendor, owner, area, lpu, lpu_image, contract_type, segment, sap_contract, contract_year,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       RETURNING id, title, vendor, owner, area, lpu, lpu_image, contract_type, segment, sap_contract,
...
export function updateContract(input: ContractUpdatePayload) {
  ...
  const record = db
    .prepare<ContractRecord>(
      `UPDATE contracts
       SET title = ?, vendor = ?, owner = ?, area = ?, lpu = ?, lpu_image = ?, contract_type = ?, segment = ?,
